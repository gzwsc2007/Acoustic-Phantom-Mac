...
#define SERVO_SERVER_HOST  "172.24.1.1"
...
*/
    ...
    int frame_width = im0_gray.cols;
    int frame_height = im0_gray.rows;
    float img_center_x = 0.5 * frame_width;
    float img_center_y = 0.5 * frame_height;
    ...
    while (true)
    {
        ...
        // Thresh
        if (cmt.points_active.size() > 20) {
            float d_pixel_x = img_center_x - cmt.bb_rot.center.x;
            float d_pixel_y = img_center_y - cmt.bb_rot.center.y;
            float ddeg[2];
            ddeg[0] = PIXEL_TO_DEG_X * d_pixel_x;
            ddeg[1] = PIXEL_TO_DEG_Y * d_pixel_y;
            cout << "Move Deg: " << ddeg[0] << "\t" << ddeg[1] << "\t";
